...
int main(int argc, const char *argv[]) {
  ...
  cv::Mat P_rect_00(3, 4,
                    cv::DataType<double>::type); // 3x4 projection matrix after
                                                 // rectification
  cv::Mat R_rect_00(
      4, 4, cv::DataType<double>::type); // 3x3 rectifying rotation to make
                                         // image planes co-planar
  cv::Mat RT(
      4, 4,
  ...
  // misc
  double sensorFrameRate = 10.0 / imgStepWidth; // frames per second for Lidar
                                                // and camera
