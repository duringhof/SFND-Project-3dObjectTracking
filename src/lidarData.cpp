...
void showLidarImgOverlay(cv::Mat &img, std::vector<LidarPoint> &lidarPoints, cv::Mat &P_rect_xx, cv::Mat &R_rect_xx, cv::Mat &RT, cv::Mat *extVisImg, bool Red)
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
            ...
            int green =
                min(255, (int)(255 * (1 - abs((val - maxVal) / maxVal))));
            if (Red == true) {
